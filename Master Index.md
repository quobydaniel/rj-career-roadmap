---
created: 2026-08-01
type: master-index
status: active
tags: [index, navigation, master-reference]
---

# 📚 Obsidian Roadmap — Master Index

> **Start here.** This is your single point of entry to everything in your system.
> If you're lost or forget "what do I do next" → open this file first.
> Every document links to the others. One door at a time.
> See also: [[System Inventory & Recovery Manual]]

---

## 🎯 WHERE TO GO RIGHT NOW (based on what you need)

| If you want to... | Open this |
|---|---|
| Understand the whole journey | [[RJ Master Plan — One Rep a Day]] |
| Know how Door 1 feeds the Saudi job | [[Career Stack — One Action, Three Returns]] |
| Pitch a client today | [[Pitch Script]] + [[Door 1 — First Pitch (held by hand)]] |
| Find more clients | [[How to Find Clients — Door 1]] |
| Log today's results | [[Fire Drill Tracker]] |
| Build the dashboard (skill) | [[Lesson - Build Your First Dashboard (Google Sheets)]] |
| Restart from a laptop loss | [[System Inventory & Recovery Manual]] |
| Park a new shiny idea | [[Idea Vault — Parked Ideas]] |

---

## 📦 THE FULL ROADMAP FOLDER (every file, what it does)

| File | Type | Purpose |
|---|---|---|
| `RJ Master Plan — One Rep a Day.md` | Strategy | The whole 3-door plan (survive → reps → launch) |
| `Career Stack — One Action, Three Returns.md` | Strategy | How every Door 1 action feeds GitHub + LinkedIn + Saudi job |
| `Fire Drill Tracker.md` | Tracker | Your daily scoreboard (log every contact, every GHS in) |
| `Pitch Script.md` | Script | Word-for-word what to say in shops + WhatsApp |
| `Door 1 — First Pitch (held by hand).md` | Guide | Day-1 minute-by-minute, with objection handlers |
| `How to Find Clients — Door 1.md` | Guide | Where to walk in Accra, who to target, the 14-day hunt table |
| `Lesson - Build Your First Dashboard (Google Sheets).md` | Lesson | 6-step skill build (the "practicals" cure) |
| `System Inventory & Recovery Manual.md` | Manual | How to recreate everything if you lose your laptop |
| `Idea Vault — Parked Ideas.md` | Vault | Where new shiny ideas wait until their time comes |
| `Phase 0 - Raw Sales Data.csv` | Data | Practice data (60 rows of pharmacy sales) |
| `Sample Dashboard - ABC Pharmacy.xlsx` | Sample | Pre-built sample (data layer only, for printing) |
| `My First Dashboard.ods` | In progress | YOUR build (Step 1 in progress) |
| `Master Index.md` | THIS FILE | Navigation hub |

---

*Last updated: 2026-08-01*
